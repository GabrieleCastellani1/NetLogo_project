...
turtles-own [
  ...
  action
  next-state
  emission-rate     ; New variable for individual car emissions
  prev-speed        ; New variable to track speed changes
  car-type          ; New: type of car (e.g., "sedan", "truck", "sports")
  acceleration-multiplier  ; New: multiplier for acceleration
  deceleration-multiplier  ; New: multiplier for deceleration
  emission-multiplier      ; New: multiplier for emissions
]
...
to setup-cars
  set-default-shape turtles "car"  ; default shape if not changed below
  create-turtles number-of-cars [
    ...
    ;; Randomly assign a car type
    let type-choice one-of ["sedan" "truck" "sports"]
    set car-type type-choice

    ;; Set multipliers and shape based on car type
    if car-type = "sedan" [
      set acceleration-multiplier 1
      set deceleration-multiplier 1
      set emission-multiplier 1
      set shape "car"         ; built-in car shape
    ]
    if car-type = "truck" [
      set acceleration-multiplier 0.8
      set deceleration-multiplier 1.2
      set emission-multiplier 1.5
      set shape "truck"       ; built-in truck shape
    ]
    if car-type = "sports" [
      set acceleration-multiplier 1.5
      set deceleration-multiplier 0.9
      set emission-multiplier 2.5
      set shape "arrow"       ; using the arrow shape as a proxy for a sports car
    ]
    separate-cars
  ]
end
...
to accelerate
  set speed speed + acceleration * acceleration-multiplier
end
...
"" ""
PENS
"Current Emissions" 1.0 0 -2674135 true "" "plot current-tick-emissions"
"Average Emissions" 1.0 0 -13840069 true "" "plot total-emissions / (ticks + 1)"
